...
#define DBG(var, type, msg) \
    char *p = V##type##_to_cstr((var)); \
    printf((msg "%s\n"), p); free(p)

ZResult VString_deep_copy(void *dest, const void *src, void *ctx) {
    (void)ctx;
...
void VString_drop(void *elem, void *ctx) {
    (void)ctx;
    VString *string = elem;
...
VPathResult VPath_copy(VPath *dest, const VPath *src) {
    ...
    ZResult res = ZDeque_copy(src->_segments, &dest->_segments);
    if (res != Z_OK)
